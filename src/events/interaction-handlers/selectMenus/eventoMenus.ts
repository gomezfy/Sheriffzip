 ...
 */
export function createEventoSelectMenu(): ActionRowBuilder<StringSelectMenuBuilder> {
 ...
 */
export async function handleEventoSelectMenu(interaction: StringSelectMenuInteraction): Promise<void> {
 ...
 */
async function handleMiningClassification(interaction: StringSelectMenuInteraction): Promise<void> {
  ...
  description += miningEvent.active 
    ? `${timerEmoji} **${formatTimeRemaining(timeLeft)}** restante\n📊 Fase ${miningEvent.phase + 1}/7\n\n` 
    : `⏸️ Evento encerrado\n\n`;
 ...
 */
async function handleMiningPrizes(interaction: StringSelectMenuInteraction): Promise<void> {
 ...
 */
async function handleHuntingClassification(interaction: StringSelectMenuInteraction): Promise<void> {
  const huntingEvent = getCurrentHuntingEvent();
  const leaderboard = getHuntingEventLeaderboard().slice(0, 10);
  ...
  const bearPeltEmoji = getBearPeltEmoji();
 ...
 */
async function handleHuntingPrizes(interaction: StringSelectMenuInteraction): Promise<void> {
  const trophyEmoji = getTrophyEmoji();
  const goldMedalEmoji = getGoldMedalEmoji();
  const silverMedalEmoji = getSilverMedalEmoji();
  const bronzeMedalEmoji = getBronzeMedalEmoji();
