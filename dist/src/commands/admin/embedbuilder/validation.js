...
exports.validateEmbedLimits = validateEmbedLimits;
...
function validateImageURL(url) {
    ...
    const isCommonImageHost = lowerUrl.includes("i.redd.it") ||
...
function validateEmbedLimits(embedData) {
    ...
    const totalCharacters = (embedData.title?.length || 0) +
        (embedData.description?.length || 0) +
        embedData.fields.reduce((sum, field) => sum + field.name.length + field.value.length, 0) +
