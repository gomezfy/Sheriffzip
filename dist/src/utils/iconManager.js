...
exports.ICON_MAPPINGS = void 0;
exports.getIconEmoji = getIconEmoji;
exports.uploadIconsToGuild = uploadIconsToGuild;
exports.listAvailableIcons = listAvailableIcons;
 ...
 */
function getIconEmoji(action, client) {
    const mapping = exports.ICON_MAPPINGS.find((m) => m.action === action);
    if (!mapping) {
 ...
 */
async function uploadIconsToGuild(client, guildId) {
    const guild = client.guilds.cache.get(guildId);
    if (!guild) {
        throw new Error(`Guild ${guildId} não encontrada`);
    }
    const iconsPath = path_1.default.join(process.cwd(), "assets", "icons");
    if (!fs_1.default.existsSync(iconsPath)) {
    ...
    const uniqueIcons = new Set(exports.ICON_MAPPINGS.map((m) => m.iconName));
    let uploaded = 0;
    let skipped = 0;
    for (const iconName of uniqueIcons) {
        const iconPath = path_1.default.join(iconsPath, `${iconName}.svg`);
        ...
        try {
            ...
            const existingEmoji = guild.emojis.cache.find((e) => e.name === `feather_${iconName.replace(/-/g, "_")}`);
            if (existingEmoji) {
...
function listAvailableIcons() {
    ...
    exports.ICON_MAPPINGS.forEach((mapping) => {
    ...
    console.log(`\n📊 Total: ${exports.ICON_MAPPINGS.length} ícones mapeados`);
}
/**
...
function getIconPath(action) {
    ...
    return path_1.default.join(process.cwd(), "assets", "icons", `${mapping.iconName}.svg`);
}
