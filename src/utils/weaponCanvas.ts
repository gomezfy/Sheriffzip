...
function drawWoodTexture(ctx: any, x: number, y: number, width: number, height: number) {
...
function drawRope(ctx: any, x1: number, y1: number, x2: number, y2: number) {
  const segments = 15;
...
function drawStar(ctx: any, x: number, y: number, size: number, color: string) {
  ctx.save();
  ctx.fillStyle = color;
  ctx.beginPath();
  for (let i = 0; i < 5; i++) {
...
export async function generateWeaponCard(weapon: WeaponData): Promise<Buffer> {
  ...
  CanvasOptimizer.roundRect(ctx, 60, 40, width - 120, 90, 12);
  ctx.fill();
  ...
  try {
    ...
    CanvasOptimizer.roundRect(ctx, bgX, bgY, bgSize, bgSize, 20);
    ctx.fill();
    ...
    ctx.strokeStyle = "#8B6F47";
    ctx.lineWidth = 2;
    CanvasOptimizer.roundRect(ctx, bgX + 5, bgY + 5, bgSize - 10, bgSize - 10, 18);
    ctx.stroke();
  ...
  CanvasOptimizer.roundRect(ctx, 100, statsY - 50, width - 200, 120, 18);
  ctx.fill();
  ...
  ctx.strokeStyle = "#8B6F47";
  ctx.lineWidth = 2;
  CanvasOptimizer.roundRect(ctx, 105, statsY - 45, width - 210, 110, 16);
  ctx.stroke();
