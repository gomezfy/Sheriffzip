...
const serverTemplates_1 = require("../../utils/serverTemplates");
async function cleanupServer(interaction, skipChannelId) {
    ...
    const skipChannel = await guild.channels
        ...
        .catch(() => null);
    if (!skipChannel) {
        console.warn("⚠️ Could not fetch interaction channel for cleanup - aborting cleanup for safety");
        throw new Error("Could not fetch interaction channel. Cleanup aborted for safety.");
    }
        ...
        try {
            if (channel.type === discord_js_1.ChannelType.GuildCategory) {
                if (id === skipCategoryId) {
                    console.log(`Skipping category ${channel.name} because it contains the interaction channel`);
                    continue;
                }
...
async function createServerFromTemplate(interaction, template) {
    const guild = interaction.guild;
    const createdRoles = [];
        ...
        try {
            ...
            for (const channelData of categoryData.channels) {
                try {
                    ...
                    createdChannels.push(channelData.type === discord_js_1.ChannelType.GuildVoice
                        ? `🔊 ${channel.name}`
                        : `#${channel.name}`);
                    await new Promise((resolve) => setTimeout(resolve, 300));
                }
...
module.exports = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .setDescription("🏛️ Cria estrutura profissional de servidor com templates prontos")
    ...
    async execute(interaction) {
        ...
        const templates = (0, serverTemplates_1.getAllTemplates)();
        const selectMenuEmbed = new discord_js_1.EmbedBuilder()
            .setColor(discord_js_1.Colors.Blue)
            .setTitle("🏛️ Criar Servidor - Escolha um Template")
            .setDescription("**Selecione um template profissional abaixo:**\n\n" +
            templates
        ...
        const selectMenu = new discord_js_1.StringSelectMenuBuilder()
            ...
            .addOptions(templates.map((template) => new discord_js_1.StringSelectMenuOptionBuilder()
        ...
        try {
            ...
            const selectedTemplate = (0, serverTemplates_1.getTemplateById)(selectedTemplateId);
            if (!selectedTemplate) {
            ...
            const cleanupEmbed = new discord_js_1.EmbedBuilder()
                .setColor(discord_js_1.Colors.Orange)
                .setTitle("⚠️ Limpar Servidor Existente?")
                .setDescription(`**Você quer limpar o servidor antes de criar a nova estrutura?**\n\n` +
            ...
            if (shouldCleanup) {
                const cleaningEmbed = new discord_js_1.EmbedBuilder()
                ...
                const cleanupStats = await cleanupServer(interaction, interaction.channelId);
                const cleanedEmbed = new discord_js_1.EmbedBuilder()
            ...
            const creatingEmbed = new discord_js_1.EmbedBuilder()
                .setColor(discord_js_1.Colors.Blue)
                .setTitle(`${selectedTemplate.emoji} Criando: ${selectedTemplate.name}`)
                .setDescription(`**Criando estrutura do servidor...**\n\n` +
            ...
            const result = await createServerFromTemplate(interaction, selectedTemplate);
            const successEmbed = new discord_js_1.EmbedBuilder()
                .setColor(discord_js_1.Colors.Green)
                .setTitle("✅ Servidor Criado com Sucesso!")
                .setDescription(`**Template aplicado:** ${selectedTemplate.emoji} ${selectedTemplate.name}`)
                .addFields({
                name: "👥 Cargos Criados",
...
//# sourceMappingURL=criaservidor.js.map