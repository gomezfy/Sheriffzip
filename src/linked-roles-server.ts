 ...
 */
app.get("/api/emojis", (req: Request, res: Response) => {
  try {
    ...
    for (const [key, value] of Object.entries(APPLICATION_EMOJIS)) {
      if (value && value !== "") {
        const emojiString = value as string;
...
app.get("/api/events", (req: Request, res: Response) => {
  try {
    ...
    const miningEvent = activeEvents.find((e: any) => e.type === "mining");
    const huntingEvent = activeEvents.find((e: any) => e.type === "hunting");
    ...
    const miningLeaderboardWithAvatars = miningLeaderboard.slice(0, 10).map((player: any) => ({
      ...player,
      avatarUrl: `https://cdn.discordapp.com/embed/avatars/${parseInt(player.userId) % 6}.png`
    }));

    const huntingLeaderboardWithAvatars = huntingLeaderboard.slice(0, 10).map((player: any) => ({
      ...player,
      avatarUrl: `https://cdn.discordapp.com/embed/avatars/${parseInt(player.userId) % 6}.png`
    }));
