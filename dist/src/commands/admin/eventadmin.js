...
exports.default = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addChoices({ name: "Todos", value: "all" }, { name: "Mineração", value: "mining" }, { name: "Caça", value: "hunting" })
        .setRequired(false)))
        .addSubcommand((sub) => sub
        .setName("status")
...
async function handleStartMining(interaction) {
    ...
    if (currentEvent && currentEvent.active) {
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setDescription(`Já existe um evento de mineração ativo!\n\n` +
    ...
    // Start the event
    const event = await (0, eventManager_1.startMiningEvent)(interaction.client, undefined, name);
    // Update end time if custom duration
    if (days !== 2 || hours !== 0) {
        const data = (0, eventManager_1.getEventsData)();
        if (data.currentEvent) {
            data.currentEvent.endTime = endTime;
            (0, eventManager_1.saveEventsData)(data);
        }
    }
    const embed = new discord_js_1.EmbedBuilder()
        .setColor(0xFFD700)
        .setTitle("⛏️ Evento de Mineração Iniciado!")
        .setDescription(`O evento de mineração foi iniciado com sucesso!\n\n` +
        `**Nome:** ${name}\n` +
...
async function handleStartHunting(interaction) {
    const days = interaction.options.getInteger("days") || 2;
    const hours = interaction.options.getInteger("hours") || 0;
    const name = interaction.options.getString("name") || "Caçada do Oeste";
    // Check if hunting event already active
    const currentEvent = (0, eventManager_1.getCurrentHuntingEvent)();
    if (currentEvent && currentEvent.active) {
        const embed = new discord_js_1.EmbedBuilder()
            .setColor(0xFFA500)
            .setTitle("⚠️ Evento Já Ativo")
            .setDescription(`Já existe um evento de caça ativo!\n\n` +
            `**Nome:** ${currentEvent.name}\n` +
            `**Início:** <t:${Math.floor(currentEvent.startTime / 1000)}:F>\n` +
            `**Término:** <t:${Math.floor(currentEvent.endTime / 1000)}:F>\n\n` +
    ...
    const durationMs = (days * 24 * 60 * 60 * 1000) + (hours * 60 * 60 * 1000);
    // Start the event
    const event = await (0, eventManager_1.startHuntingEvent)(interaction.client, undefined, name, durationMs);
    const embed = new discord_js_1.EmbedBuilder()
        .setColor(0x8B4513)
        .setTitle("🎯 Evento de Caça Iniciado!")
        .setDescription(`O evento de caça foi iniciado com sucesso!\n\n` +
        `**Nome:** ${name}\n` +
        `**Duração:** ${days} dia(s) e ${hours} hora(s)\n` +
        `**Início:** <t:${Math.floor(event.startTime / 1000)}:F>\n` +
...
async function handleStopEvents(interaction) {
...
async function handleStatus(interaction) {
    const activeEvents = (0, eventManager_1.getAllActiveEvents)();
