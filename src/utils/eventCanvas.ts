import { createCanvas, GlobalFonts, SKRSContext2D, loadImage, Image } from "@napi-rs/canvas";
 ...
 */
async function loadEmojiImage(filename: string): Promise<Image | null> {
  try {
    if (imageCache.has(filename)) {
      return imageCache.get(filename)!;
    }
    
    const imagePath = path.join(process.cwd(), "assets/emojispng", filename);
...
// Registrar fontes
const fontsRegistered = GlobalFonts.families.some(f => f.family === "Nunito");
if (!fontsRegistered) {
  try {
...
export async function createEventCanvas(data: EventCanvasData): Promise<Buffer> {
  const width = 1400;
  const height = 1600;
...
function drawMiningEvent(
  ctx: SKRSContext2D,
  event: NonNullable<EventCanvasData["miningEvent"]>,
  startY: number
): number {
  const boxWidth = 1300;
  const boxX = 50;
  ...
  for (let i = 0; i < leaderboard.length; i++) {
    ...
    if (i < 3) {
      ...
      if (i === 0) ctx.strokeStyle = "rgba(255, 215, 0, 0.4)";
      else if (i === 1) ctx.strokeStyle = "rgba(192, 192, 192, 0.4)";
      else ctx.strokeStyle = "rgba(205, 127, 50, 0.4)";
      ctx.lineWidth = 2;
      drawRoundRect(ctx, boxX + 30, rowY - 40, boxWidth - 60, 52, 15);
      ctx.stroke();
    } else if (i % 2 === 0) {
...
function drawHuntingEvent(
  ctx: SKRSContext2D,
  event: NonNullable<EventCanvasData["huntingEvent"]>,
  startY: number
): number {
  const boxWidth = 1300;
  const boxX = 50;
...
function drawNextEvent(
  ctx: SKRSContext2D,
  nextEvent: NonNullable<EventCanvasData["nextEvent"]>,
  startY: number
): void {
  const boxWidth = 1120;
  const boxX = 40;
  ...
  const alarmEmoji = getEmojiForCanvas("alarm");
  const clockEmoji = getEmojiForCanvas("clock");
  ctx.fillStyle = "#FF6B00";
  ctx.font = "bold 42px Nunito Bold, Arial";
  ctx.textAlign = "center";
  ...
  const timerEmoji = getEmojiForCanvas("timer");
  ctx.fillStyle = "#28A745";
  ctx.font = "bold 34px Nunito Bold, Arial";
 ...
 */
export async function createEventsOverviewCanvas(
  miningActive: boolean,
  miningTimeRemaining: string,
  huntingActive: boolean,
  huntingTimeRemaining: string,
  nextEventType: "mining" | "hunting",
  nextEventName: string,
  nextEventDate: string,
  nextEventTimeUntil: string
): Promise<Buffer> {
 ...
 */
export async function createPrizesCanvas(eventType: "mining" | "hunting"): Promise<Buffer> {
  const width = 1200;
  const height = 1000;
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext("2d");
  ...
  const eventIcon = eventType === "mining" ? getEmojiForCanvas("pickaxe") : getEmojiForCanvas("dart");
  const sparkles = getEmojiForCanvas("sparkles");
  ...
  for (const prize of prizes) {
    ...
    let medal = "";
    if (prize.position === 1) medal = getEmojiForCanvas("gold_medal");
    else if (prize.position === 2) medal = getEmojiForCanvas("silver_medal");
    else if (prize.position === 3) medal = getEmojiForCanvas("bronze_medal");
    else medal = `${prize.position}º`;
    ...
    const silverCoin = getEmojiForCanvas("silver_coin");
    const saloonToken = getEmojiForCanvas("saloon_token");
    const sparklesReward = getEmojiForCanvas("sparkles");
 ...
 */
export async function createClassificationCanvas(
  eventType: "mining" | "hunting",
  leaderboard: Array<{
    position: number;
    username: string;
    points: number;
  ...
  // Título com emoji
  const eventIcon = eventType === "mining" ? getEmojiForCanvas("pickaxe") : getEmojiForCanvas("dart");
  const sparklesIcon = getEmojiForCanvas("sparkles");
  ...
  if (top10.length === 0) {
    ...
    for (let i = 0; i < top10.length; i++) {
      ...
      let medal = "";
      if (i === 0) medal = getEmojiForCanvas("gold_medal");
      else if (i === 1) medal = getEmojiForCanvas("silver_medal");
      else if (i === 2) medal = getEmojiForCanvas("bronze_medal");
      else medal = `${i + 1}º`;
