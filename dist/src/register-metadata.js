...
async function getBotAccessToken() {
    if (!DISCORD_CLIENT_ID || !DISCORD_CLIENT_SECRET) {
        throw new Error("DISCORD_CLIENT_ID and DISCORD_CLIENT_SECRET must be set in environment variables");
    }
    ...
    try {
        const response = await axios_1.default.post(`${DISCORD_API}/oauth2/token`, params.toString(), {
            auth: {
        ...
        console.error("❌ Error getting bot access token:", error.response?.data || error.message);
        throw error;
    }
}
/**
...
async function registerMetadata() {
    ...
    try {
        ...
        const response = await axios_1.default.put(`${DISCORD_API}/applications/${DISCORD_CLIENT_ID}/role-connections/metadata`, METADATA_SCHEMA, {
            headers: {
        ...
        console.log("      OAuth2 → Redirects → Add: http://your-domain.com/discord/callback");
...
async function viewMetadata() {
    ...
    try {
        const accessToken = await getBotAccessToken();
        const response = await axios_1.default.get(`${DISCORD_API}/applications/${DISCORD_CLIENT_ID}/role-connections/metadata`, {
            headers: {
                Authorization: `Bearer ${accessToken}`,
            },
        });
        if (response.data.length === 0) {
            console.log("⚠️  No metadata registered yet. Run without --view flag to register.");
        }
        else {
