...
exports.createStoreItemCanvas = createStoreItemCanvas;
...
async function loadEmojiImage(emojiKey) {
    const emojiCode = customEmojis_1.APPLICATION_EMOJIS[emojiKey];
    if (!emojiCode)
        return null;
    ...
    if (!url)
        return null;
    try {
        return await canvasCache_1.canvasCache.loadImageWithCache(url);
    }
    catch (error) {
...
function ensureFontsRegistered() {
    if (fontsRegistered)
        return;
    const fontsDir = path_1.default.join(process.cwd(), 'assets', 'fonts');
    const fontFiles = [
    ...
    for (const { file, family } of fontFiles) {
        const fontPath = path_1.default.join(fontsDir, file);
...
async function createStoreItemCanvas(item, userTokens, userHasItem) {
    ...
    if (item.imageFile) {
        if (item.imageFile.startsWith('http://') || item.imageFile.startsWith('https://')) {
            try {
                itemImage = await canvasCache_1.canvasCache.loadImageWithCache(item.imageFile);
            }
            catch (error) {
            ...
            const imagePath = path_1.default.join(process.cwd(), 'assets', 'shop-items', item.imageFile);
            if (fs_1.default.existsSync(imagePath)) {
                try {
                    itemImage = await canvasCache_1.canvasCache.loadImageWithCache(imagePath);
                }
                catch (error) {
    ...
    if (itemImage) {
        ctx.save();
        const glowGradient = ctx.createRadialGradient(imageX + imageSize / 2, imageY + imageSize / 2, imageSize / 3, imageX + imageSize / 2, imageY + imageSize / 2, imageSize / 1.5);
...
//# sourceMappingURL=generalStoreCanvas.js.map