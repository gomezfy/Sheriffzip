...
exports.createHuntingCanvas = createHuntingCanvas;
exports.createHuntingStartCanvas = createHuntingStartCanvas;
const canvas_1 = require("@napi-rs/canvas");
const path_1 = __importDefault(require("path"));
const canvasCache_1 = require("./canvasCache");
// Helper function to get pelt image path
function getPeltImagePath(peltId) {
    const peltImages = {
    ...
    if (!fileName)
        return null;
    return path_1.default.join(process.cwd(), "assets", "custom-emojis", "pelts", fileName);
}
async function createHuntingCanvas(result, userName) {
    const width = 800;
    const height = 600;
    ...
    if (result.success && result.animal) {
        ...
        if (result.rewards) {
            ...
            if (result.rewards.meat) {
                ctx.textAlign = "left";
                ctx.fillText(`🥩 ${result.rewards.meat.name} x${result.rewards.meat.amount}`, 80, rewardY);
                rewardY += 30;
            }
            if (result.rewards.pelt) {
                ...
                if (peltImagePath) {
                    try {
                        ...
                        const peltImage = await canvasCache_1.canvasCache.loadImageWithCache(peltImagePath);
                        ...
                        ctx.restore();
                        ctx.fillText(`${result.rewards.pelt.name} x${result.rewards.pelt.amount}`, 80 + emojiSize + 8, rewardY);
                    }
                    catch (error) {
                        // Fallback to text emoji if image fails to load
                        ctx.fillText(`${result.animal.emoji} ${result.rewards.pelt.name} x${result.rewards.pelt.amount}`, 80, rewardY);
                    }
                }
                else {
            ...
            if (result.rewards.feather) {
                ctx.fillText(`🪶 ${result.rewards.feather.name} x${result.rewards.feather.amount}`, 80, rewardY);
            }
            // XP gained
    ...
    if (result.attemptsRemaining !== undefined && result.maxAttempts !== undefined) {
        ...
        ctx.font = "20px Arial";
        ctx.textAlign = "center";
        ctx.fillText(`🎯 Tentativas: ${result.maxAttempts - result.attemptsRemaining}/${result.maxAttempts}`, width - 100, 140);
    }
    return canvas.toBuffer("image/png");
}
async function createHuntingStartCanvas(animalName, animalEmoji, animalRarity, animalRarityColor, userName, attemptsRemaining, maxAttempts) {
    const width = 800;
    const height = 600;
    const canvas = (0, canvas_1.createCanvas)(width, height);
    const ctx = canvas.getContext("2d");
...
//# sourceMappingURL=huntingCanvas.js.map