...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    await interaction.deferReply({ 
      flags: isInGuild ? MessageFlags.Ephemeral : undefined 
    });
...
async function createProfileCard(
  ...
  if (stats.background) {
    ...
    if (bgData?.imageUrl) {
      try {
        const bgImage = await canvasCache.loadImageWithCache(bgData.imageUrl);
    ...
    if (!backgroundLoaded) {
      try {
        ...
        if (fs.existsSync(bgPath)) {
          const bgImage = await canvasCache.loadImageWithCache(bgPath);
          ctx.drawImage(bgImage, 0, 0, 1536, 1024);
          backgroundLoaded = true;
        }
      } catch (error) {
  ...
  if (!backgroundLoaded) {
    try {
      const defaultBgImage = await canvasCache.loadImageWithCache(
  ...
  try {
    ...
    const avatar = await canvasCache.loadImageWithCache(avatarURL);
    ...
    if (userFrameUrl) {
      try {
        const frame = await canvasCache.loadImageWithCache(userFrameUrl);
        const frameSize = 300;
  ...
  async function drawStat(
    ...
    if (emojiPath) {
      try {
        const emojiImg = await canvasCache.loadImageWithCache(emojiPath);
...
async function drawLineWithEmojis(
  ...
  for (const part of parts) {
    if (part.type === "text") {
      ...
      try {
        const emojiImg = await canvasCache.loadImageWithCache(part.path);
        ctx.drawImage(
          emojiImg,
          currentX,
          y - emojiSize + 4,
